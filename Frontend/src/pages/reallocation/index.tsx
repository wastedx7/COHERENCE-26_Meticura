    ...
    return (
        <div className="flex flex-col gap-6 animate-fade-in pb-12">
            <div className="flex justify-between items-end mb-2">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Reallocation Engine</h1>
                    <p className="text-slate-500 mt-1">Review, approve, and execute AI-suggested budget transfers</p>
                </div>
            </div>

            <div className="glass-panel p-6">
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-4">Transfer Lifecycle Summary</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 select-none relative z-10">
                    {[
                        { id: 'pending', label: 'Pending Review', icon: Clock, count: summary?.pending || 12, amount: 4200000, color: 'text-amber-600', bg: 'bg-amber-50' },
                        { id: 'approved', label: 'Approved (Ready)', icon: CheckCircle2, count: summary?.approved || 5, amount: 1500000, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                        { id: 'rejected', label: 'Rejected', icon: XCircle, count: summary?.rejected || 3, amount: 800000, color: 'text-rose-600', bg: 'bg-rose-50' },
                        { id: 'executed', label: 'Executed', icon: PlayCircle, count: summary?.executed || 24, amount: 12500000, color: 'text-emerald-600', bg: 'bg-emerald-50' }
                    ].map((item) => (
                        <div
                            key={item.id}
                        ...
                        >
                            ...
                            <p className="text-xs text-slate-500 mt-1 font-medium">₹{(item.amount / 100000).toFixed(1)}L Total Volume</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="glass-card overflow-hidden border border-slate-200 shadow-sm mt-4">
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead className="bg-slate-50/80 backdrop-blur-sm border-b border-slate-200">
                        <tr>
                            <th className="px-6 py-4 font-semibold text-slate-600">Transfer Path</th>
                            <th className="px-6 py-4 font-semibold text-slate-600">Suggested Amount</th>
