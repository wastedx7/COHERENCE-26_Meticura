    ...
    return (
        <div className="animate-fade-in pb-12">
            <div className="mb-6 flex justify-between items-end">
                ...
                >
                    {isLoading ? t('common.loading') : t('users.refreshUsers')}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="glass-panel p-6 flex flex-col text-center items-center justify-center bg-indigo-50/50">
                    ...
                    <p className="text-sm text-slate-500 font-medium">{t('users.stats.distinctRoles')}</p>
                </div>
                <div className="glass-panel p-6 flex flex-col text-center items-center justify-center">
                    ...
                    <p className="text-sm text-slate-500 font-medium">{t('users.stats.totalRegistered')}</p>
                </div>
                <div className="glass-panel p-6 flex flex-col text-center items-center justify-center">
                    <UserCheck className="w-8 h-8 text-emerald-500 mb-2" />
                    <h3 className="text-2xl font-bold text-slate-800">{activeUsers}</h3>
                    <p className="text-sm text-slate-500 font-medium">{t('users.stats.activeAccounts')}</p>
                </div>
            </div>
                ...
                {isLoading && users.length === 0 ? (
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                                <th className="px-6 py-4 font-semibold text-slate-600">{t('users.table.userDetails')}</th>
                                <th className="px-6 py-4 font-semibold text-slate-600">{t('users.table.assignedRole')}</th>
                                <th className="px-6 py-4 font-semibold text-slate-600">{t('users.table.status')}</th>
                                <th className="px-6 py-4 text-right font-semibold text-slate-600">{t('users.table.actions')}</th>
                            </tr>
                        </thead>
